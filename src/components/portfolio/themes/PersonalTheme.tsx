      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border/50">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <button onClick={() => scrollTo('story')} className="text-sm hover:text-accent transition-colors">My Story</button>
            <button onClick={() => scrollTo('journey')} className="text-sm hover:text-accent transition-colors">Journey</button>
            <button onClick={() => scrollTo('works')} className="text-sm hover:text-accent transition-colors">Works</button>
            <button onClick={() => scrollTo('connect')} className="text-sm hover:text-accent transition-colors">Connect</button>
            <ThemeToggle />
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden bg-background border-b border-border px-6 py-4 space-y-3">
            <button onClick={() => scrollTo('story')} className="block w-full text-left py-2">My Story</button>
            <button onClick={() => scrollTo('journey')} className="block w-full text-left py-2">Journey</button>
            <button onClick={() => scrollTo('works')} className="block w-full text-left py-2">Works</button>
            <button onClick={() => scrollTo('connect')} className="block w-full text-left py-2">Connect</button>
          </div>
        )}
      </nav>

      {/* Hero - Personal & Elegant */}
      <section id="story" className="min-h-screen flex items-center pt-20 relative overflow-hidden">
        ...
        <section id="journey" className="py-24 px-6 bg-muted/30">
          <div className="container mx-auto max-w-4xl">
            <div className="text-center mb-16">
              ...
              <h2 className="text-3xl font-serif mb-4">My Journey</h2>
              <p className="text-muted-foreground font-light">The path that shaped who I am today</p>
            </div>

            <div className="space-y-16">
              {experiences.length > 0 && (
                <div>
                  <h3 className="text-xl font-serif mb-8 text-center flex items-center justify-center gap-3">
                    <Briefcase className="w-5 h-5 text-primary" />
                    Career Path
                  </h3>
                  <div className="space-y-8">
                    {experiences.map((exp, index) => (
                      <div key={exp.id} className={`flex gap-8 ${index % 2 === 0 ? '' : 'flex-row-reverse'}`}>
                        ...
                        <Card className="flex-1 md:w-1/2">
                          <CardContent className="p-6">
                            ...
                            {exp.description && <p className="text-sm mt-3 text-muted-foreground">{exp.description}</p>}
                          </CardContent>
                        </Card>
                      </div>
                    ))}
      ...
      {skills.length > 0 && (
        <section className="py-24 px-6">
          <div className="container mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-serif mb-12">What I Do</h2>
            <div className="flex flex-wrap justify-center gap-3">
              {skills.map((skill) => (
                <Badge key={skill.id} variant="outline" className="px-4 py-2 text-sm font-light rounded-full">
                  {skill.name}
                </Badge>
              ))}
            </div>
      ...
      {projects.length > 0 && (
        <section id="works" className="py-24 px-6 bg-muted/30">
          <div className="container mx-auto">
            <div className="text-center mb-16">
              <h2 className="text-3xl font-serif mb-4">My Works</h2>
              <p className="text-muted-foreground font-light">Things I've created with passion</p>
            </div>
                ...
                {socialLinks.map((link) => {
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-12 h-12 rounded-full bg-muted flex items-center justify-center hover:bg-accent hover:text-accent-foreground transition-all"
                    >
                      <Icon className="w-5 h-5" />
                    </a>
                  );
