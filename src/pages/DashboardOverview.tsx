  ...
  return (
    <div className="space-y-4 animate-fade-in text-foreground">
      ...
      <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="h-9 border-border text-foreground hover:bg-muted" onClick={() => navigate("/dashboard/edit")}>
              <FileEdit className="w-4 h-4 mr-1.5" /> Edit
            </Button>
            {profile && (
              <Button size="sm" className="h-9 bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => window.open(portfolioUrl, "_blank")}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className={cn("text-xs px-2 py-0.5 rounded-full border", portfolio?.is_published ? "border-primary/40 text-primary bg-primary/5" : "border-border text-muted-foreground")}>
              {portfolio?.is_published ? "Live" : "Draft"}
            </span>
            <button disabled={saving} onClick={togglePublish} className="text-xs text-muted-foreground hover:text-foreground inline-flex items-center">
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        ...
        {score < 100 && (
          <Button className="w-full mt-4 h-9 text-sm bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => navigate("/dashboard/edit")}>
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-lg border border-border bg-muted/50 grid place-items-center">
        ...
        <Button size="sm" className="h-9 bg-primary text-primary-foreground hover:bg-primary/90 shrink-0" onClick={() => navigate("/#pricing")}>
      ...
      {profile && (
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-lg border border-border bg-muted/50 grid place-items-center"><Globe className="w-5 h-5 text-foreground" /></div>
            <div className="min-w-0">
              <p className="font-medium text-sm">Your Portfolio</p>
              <p className="text-xs text-muted-foreground truncate">{portfolioUrl}</p>
            </div>
          </div>
          <Button variant="outline" size="sm" className="h-8 text-xs border-border text-foreground hover:bg-muted" onClick={() => {
      ...
      {/* Support - colorful */}
      <div
        className="rounded-xl p-4 flex items-center justify-between gap-3 border border-emerald-500/30"
        style={{ background: "linear-gradient(135deg, rgba(16,185,129,0.12), rgba(59,130,246,0.12) 60%, rgba(168,85,247,0.12))" }}
      >
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg grid place-items-center shadow-sm" style={{ background: "linear-gradient(135deg, #25D366, #128C7E)" }}>
        ...
        <Button
          className="h-9 text-white border-0 hover:opacity-90 shadow-sm"
    ...
    <div className="rounded-xl border border-border bg-card p-4 shadow-sm space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg border border-border bg-muted/40 grid place-items-center text-foreground">{icon}</div>
